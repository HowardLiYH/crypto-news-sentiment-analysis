'''
News Title Abstraction for Blockworks

'''
import sys
import os

# Calculate the path to the parent directory
parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# Add the parent directory to sys.path
sys.path.append(parent_dir)

# Initiate the news scraping
from connection_func import connection

def get_news_titles():
    '''
    Inputs:
        url: the target url link of the news website
        name: the name of the news website
    Output:
        news_collector: all the news titles obtainable from the site
    '''
    news_collector = []
    total_url = ['https://blockworks.co/category/markets', 'https://blockworks.co/category/policy', 'https://blockworks.co/']
    for i in enumerate(total_url):
        url = i[1]
        name = 'Blockworks'
        soup = connection(url, name)

        if i[0] == 0:
            # Markets News Explorer
            explorer_titles = soup.find_all('div', class_='flex justify-start items-start self-stretch flex-grow-0 flex-shrink-0 gap-2.5')


            # Extract the text or title from each span
            for title in explorer_titles:
                news_collector.append(title.text)

        if i[0] == 1:
            # News Explorer
            explorer_titles = soup.find_all('a', class_='font-headline')


            # Extract the text or title from each span
            for title in explorer_titles:
                news_collector.append(title.text)

        if i[0] == 2:
            # News Explorer
            explorer_titles = soup.find_all('span', class_='text-sm')


            # Extract the text or title from each span
            for title in explorer_titles:
                news_collector.append(title.text)


    print(f'Total of {len(news_collector)} News Titles from {name} has been successfully collected!\n')
    return news_collector
